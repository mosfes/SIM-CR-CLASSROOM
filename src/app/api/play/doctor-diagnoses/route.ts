...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!doctor || !classroom || !group || !simulation) {
      return jsonError("ไม่พบข้อมูลแพทย์ ห้องเรียน หรือกลุ่ม กรุณาเลือกใหม่", 400);
    }
    if (!nurseInterview) {
      return jsonError("ผู้ป่วยรายนี้ได้รับการตรวจวินิจฉัยแล้ว หรือไม่อยู่ในกลุ่มนี้", 409);
    }
    if (!disease) {
